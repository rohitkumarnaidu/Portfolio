    ...
    <div className="min-h-dvh bg-surface-primary">
      <div className="max-w-6xl mx-auto px-4 py-16 md:py-24">
        <div className="mb-12">
          ...
          <p className="text-body text-text-secondary max-w-2xl">Media coverage, interviews, and press mentions.</p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {pressFeatures.map(p => (
              ...
              >
                {p.logoUrl ? (
                  <img src={p.logoUrl} alt={p.publication} className="w-12 h-12 rounded-xl object-contain bg-surface-elevated shrink-0" />
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-text-primary group-hover:text-accent-500 transition-colors">{p.publication}</p>
                  <p className="text-sm text-text-secondary mt-0.5">{p.title}</p>
                  {p.description && <p className="text-xs text-text-tertiary mt-1">{p.description}</p>}
                </div>
                <svg className="w-5 h-5 text-text-tertiary group-hover:text-accent-500 shrink-0 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
