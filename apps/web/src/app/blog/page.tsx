      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-8">
        ...
        <div className="flex gap-2 flex-wrap">
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-xl transition-all ${!categoryFilter ? 'bg-accent-500 text-white shadow-sm' : 'bg-surface-secondary text-text-secondary hover:bg-surface-elevated border border-border-primary'}`}
          >
            All
          </button>
          {categories.map((c) => (
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-xl transition-all ${categoryFilter === c ? 'bg-accent-500 text-white shadow-sm' : 'bg-surface-secondary text-text-secondary hover:bg-surface-elevated border border-border-primary'}`}
            >
              {c}
            </button>
          ))}
      ...
      {isLoading ? (
        <div className="max-w-3xl space-y-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="p-6 rounded-2xl bg-surface-secondary border border-border-primary animate-pulse space-y-3"
            >
        ...
        <div className="text-center py-20">
          ...
          {(search || categoryFilter) && (
            <button
              ...
              className="mt-3 text-accent-500 hover:text-accent-600 text-sm"
            >
              Clear filters
            </button>
          )}
            ...
            >
              <article className="p-6 rounded-2xl border border-border-primary bg-surface-secondary hover:bg-surface-elevated transition-all duration-200">
                <div className="flex items-center gap-4 mb-4 text-sm">
                  ...
                  <span className="text-text-tertiary">{post.readTimeMinutes} min read</span>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  {post.tags?.map((tag) => (
                    <span
                      key={tag}
                      className="px-2.5 py-1 text-xs font-medium rounded-full bg-surface-elevated text-text-tertiary border border-border-accent"
                    >
                      {tag}
                    </span>
                  ))}
