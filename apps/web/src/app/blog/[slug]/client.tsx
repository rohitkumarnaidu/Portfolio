  ...
  if (!post) {
    return (
      <div className="pt-32 pb-24 section-container text-center">
        ...
        <p className="text-body text-text-secondary mb-6">The article you&apos;re looking for doesn&apos;t exist or has been removed.</p>
        <Link href="/blog" className="text-accent-500 hover:text-accent-600">← Back to Blog</Link>
      </div>
    );
  }
    ...
    <article className="pt-32 pb-24">
      <div className="section-container max-w-3xl">
        ...
        <header className="mb-12">
          <div className="flex items-center gap-4 mb-6 text-sm flex-wrap" role="list" aria-label="Post metadata">
            <span role="listitem" className="text-text-tertiary">{formatDate(post.published_at || post.created_at)}</span>
            <span className="w-1 h-1 rounded-full bg-border-accent" aria-hidden="true" />
            <span role="listitem" className="text-text-tertiary">{post.read_time} min read</span>
            <span className="w-1 h-1 rounded-full bg-border-accent" aria-hidden="true" />
            <span role="listitem" className="text-accent-500 font-medium">{post.category}</span>
          </div>
        ...
        {post.author && (
          <div className="mt-12 pt-8 border-t border-border-primary">
            <p className="text-sm text-text-tertiary">Written by <span className="text-text-primary font-medium">{post.author}</span></p>
          </div>
        )}

        {relatedPosts.length > 0 && (
          <section className="mt-16 pt-8 border-t border-border-primary" aria-labelledby="related-heading">
            <h2 id="related-heading" className="text-h2 font-display text-text-primary mb-6">Related Articles</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4" role="list">
              {relatedPosts.map(rp => (
                <Link key={rp.id} href={`/blog/${rp.slug}`} className="group hover:no-underline" role="listitem">
                  <div className="p-4 rounded-xl bg-surface-secondary border border-border-primary hover:border-border-accent transition-all h-full">
                    <p className="text-xs text-text-tertiary mb-2">{formatDate(rp.published_at || rp.created_at)}</p>
