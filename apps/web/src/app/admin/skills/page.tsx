...
export default function SkillsPage() {
  ...
  const grouped = useMemo(() => filtered.reduce<Record<string, Skill[]>>((acc, s) => {
  ...
  const resetForm = () => { setEditingSkill(null); setShowForm(false); };
  ...
  const confirmDelete = async () => {
    ...
    } catch { addToast({ variant: 'error', title: 'Failed to delete skill' }); }
  };
  ...
  const tableColumns: Column<Skill>[] = useMemo(() => [
    {
      ...
      render: (s) => (
        <div className="flex items-center gap-2">
          ...
          {s.is_featured && <span className="text-xs px-1.5 py-0.5 rounded bg-accent-500/10 text-accent-500">Featured</span>}
        </div>
      ),
    },
    {
      ...
      render: (s) => (
        <div className="flex items-center gap-2">
          <div className="w-16 h-1.5 rounded-full bg-surface-elevated">
            <div className="h-full rounded-full bg-accent-500 transition-all" style={{ width: `${s.proficiency}%` }} />
          </div>
    ...
    },
    {
      ...
      width: '90px',
      sortable: true,
      render: (s) => <span className="text-xs text-text-tertiary">{new Date(s.created_at).toLocaleDateString()}</span>,
    },
  ], []);

  if (isLoading && !skills) {
    return <div className="space-y-3 animate-pulse">{[1,2,3,4].map(i => <div key={i} className="h-16 rounded-xl bg-surface-secondary border border-border-primary" />)}</div>;
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-body-sm text-text-secondary mt-1">Manage technical skills and proficiencies</p>
        </div>
        <div className="flex items-center gap-2">
          <button
      ...
      {viewMode === 'grid' ? (
        Object.entries(grouped).map(([category, items]) => (
          <div key={category} className="mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {items.map((skill) => (
                <div key={skill.id} className="flex items-center gap-3 p-3 rounded-xl bg-surface-secondary border border-border-primary hover:border-border-accent transition-colors group">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-text-primary">{skill.name}</span>
                      {skill.is_featured && <span className="text-xs px-1 py-0.5 rounded bg-accent-500/10 text-accent-500">Featured</span>}
                    </div>
                    <div className="w-full h-1.5 rounded-full bg-surface-elevated mt-1.5">
                      <div className="h-full rounded-full bg-accent-500 transition-all" style={{ width: `${skill.proficiency}%` }} />
                    </div>
                  </div>
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                    <button onClick={() => handleEdit(skill)} className="w-7 h-7 rounded-lg flex items-center justify-center text-text-tertiary hover:text-text-primary hover:bg-surface-elevated transition-colors" aria-label="Edit">
                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                    </button>
                    <button onClick={() => setDeleteTarget(skill)} className="w-7 h-7 rounded-lg flex items-center justify-center text-text-tertiary hover:text-semantic-error hover:bg-semantic-error-bg transition-colors" aria-label="Delete">
        ...
        <DataTable
          ...
          bulkActions={[
            {
              label: 'Bulk Delete',
              action: async (ids) => { await Promise.all(ids.map(id => deleteSkill.mutateAsync(id))); },
              variant: 'danger',
      ...
      <Modal isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Delete Skill" size="sm">
        <p className="text-sm text-text-secondary">Are you sure you want to delete &ldquo;{deleteTarget?.name}&rdquo;?</p>
