...
| Write tests                 | 5h         | test\_\*.py          | Unit tests for each service, integration test for pipeline    |
...
| Write tests                    | 4h         | test\_\*.py             | End-to-end chat flow, fallback testing            |
...
| Write tests                       | 3h         | test\_\*.py          | Chunking accuracy, refresh integrity        |
...
| -------- | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------------ |
| D-AI-001 | Adopt Claude Sonnet 4 as primary LLM with GPT-4o as fallback                       | Best quality/cost ratio for portfolio AI; strong reasoning for agent orchestration              | GPT-4 only (rejected Ã¢â‚¬â€ cost); GPT-3.5 only (rejected Ã¢â‚¬â€ quality); Claude Haiku (rejected Ã¢â‚¬â€ insufficient reasoning)                                                            | Jun 2026 | Chief AI Architect |
| D-AI-002 | Implement RAG pipeline with pgvector for knowledge retrieval                       | Grounds all responses in portfolio content; eliminates hallucination risk for factual queries   | No RAG / pure LLM (rejected Ã¢â‚¬â€ hallucination risk); external vector DB (rejected Ã¢â‚¬â€ added latency/cost); fine-tuned model (rejected Ã¢â‚¬â€ maintenance burden)                      | Jun 2026 | Chief AI Architect |
| D-AI-003 | Use FastAPI + LangChain for AI microservice                                        | Python-native AI ecosystem; LangChain provides agent abstractions, tool integration, and memory | Node.js + Vercel AI SDK (rejected Ã¢â‚¬â€ weaker agent ecosystem); standalone Python scripts (rejected Ã¢â‚¬â€ no API framework); custom framework (rejected Ã¢â‚¬â€ reinventing the wheel)    | Jun 2026 | Chief AI Architect |
| D-AI-004 | Implement session-based memory with 30-day retention, no cross-session persistence | Privacy-first: no visitor profiling; 30-day window sufficient for context                       | Permanent memory (rejected Ã¢â‚¬â€ privacy concerns, GDPR complexity); no memory (rejected Ã¢â‚¬â€ stateless experience); 90-day retention (rejected Ã¢â‚¬â€ unnecessary storage)              | Jun 2026 | Chief AI Architect |
| D-AI-005 | Design as Supervisor + Specialist multi-agent architecture                         | Scalable orchestration; each agent owns one domain; fault isolation                             | Single monolithic agent (rejected Ã¢â‚¬â€ poor specialization); micro-agent per query (rejected Ã¢â‚¬â€ coordination overhead); hierarchical agents only (rejected Ã¢â‚¬â€ inflexible routing) | Jun 2026 | Chief AI Architect |
| D-AI-006 | Enforce $10/month hard cost cap with per-query token budgets and model tiering     | Prevents cost overruns while maintaining quality for complex queries                            | Unlimited usage (rejected Ã¢â‚¬â€ budget risk); fixed per-session cost (rejected Ã¢â‚¬â€ complex to enforce); GPT-4-only (rejected Ã¢â‚¬â€ too expensive)                                      | Jun 2026 | Chief AI Architect |

## 14. Risk Register

| ID       | Risk                                                                              | Likelihood | Impact   | Mitigation                                                                                                                                 |
| -------- | --------------------------------------------------------------------------------- | ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| R-AI-001 | LLM API (OpenAI/Anthropic) outage breaks AI assistant functionality               | Low        | High     | Implement automatic model fallback (Claude Ã¢â€ â€ GPT-4o); cache common responses; graceful degradation to contact form                   |
...
| R-003 | Prompt injection Ã¢â‚¬â€ malicious visitor attempts to bypass system prompt or extract sensitive info | Medium     | High   | Input sanitization (strip special characters), output filtering (PII removal), system prompt with hardcoded boundaries, secondary LLM moderation check   |
...
| ------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| 1.0     | Jun 2026 | Initial AI Assistant Architecture Ã¢â‚¬â€ system architecture, knowledge sources, RAG pipeline, prompt architecture, memory strategy, safety & moderation, analytics, cost management, implementation roadmap | Chief AI Architect |
