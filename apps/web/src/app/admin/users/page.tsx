...
export default function UsersPage() {
    ...
    try {
      if (editingId) {
        await updateMutation.mutateAsync({
          id: editingId,
          data: { email: form.email, displayName: form.displayName, role: form.role },
        });
      ...
      addToast({ variant: 'error', title: 'Failed to save user' });
    }
  };
  ...
  const handleRoleChange = async (u: User, role: string) => {
    try {
      ...
      addToast({ variant: 'error', title: 'Failed to update role' });
    }
  };
  ...
  const columns: Column<User>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (u) => (
          <span
            className={`text-xs font-medium ${u.isActive ? 'text-emerald-500' : 'text-red-500'}`}
          >
