...
export class AnalyticsService {
  ...
  async trackEvent(dto: { eventName: string; pageUrl: string; sessionId: string; visitorId: string; userAgent?: string; ipAddress?: string; properties?: Record<string, unknown> }) {
    ...
    const existingSession = await this.prisma.analyticsSession.findUnique({ where: { sessionId: dto.sessionId } });
    if (existingSession) {
  ...
  async createSession(data: {
    sessionId: string; visitorId?: string; referrer?: string;
    utmSource?: string; utmMedium?: string; utmCampaign?: string;
    deviceType?: string; browser?: string; country?: string; city?: string;
  }) {
  ...
  async updateSession(sessionId: string, data: {
    ...
    deviceType?: string; browser?: string; country?: string; city?: string;
    pageViews?: number; durationSeconds?: number; lastActivityAt?: Date;
  }) {
    return this.prisma.analyticsSession.update({ where: { sessionId }, data });
  }

  async trackPageView(data: {
    sessionId?: string; pageUrl: string; pageTitle?: string; referrer?: string;
    scrollDepthPercent?: number; timeOnPageSeconds?: number;
    engagement?: Record<string, unknown>; viewedAt?: Date;
  }) {
  ...
  async getEvents(opts?: { eventName?: string; sessionId?: string; page?: number; perPage?: number }) {
    const where: any = {};
    if (opts?.eventName) where.eventName = opts.eventName;
    if (opts?.sessionId) where.sessionId = opts.sessionId;
    return paginateQuery(
      (args) => this.prisma.analyticsEvent.findMany({ where, orderBy: { createdAt: 'desc' }, ...args }),
  ...
  async getSummary(period?: string) {
    const now = new Date();
    const periodMs = period === '24h' ? 86400000 : period === '7d' ? 604800000 : period === '90d' ? 7776000000 : 2592000000;
    const cutoff = new Date(now.getTime() - periodMs);

    const totalPageViews = await this.prisma.analyticsEvent.count({ where: { eventName: 'page_view', createdAt: { gte: cutoff } } });
    const sessions = await this.prisma.analyticsSession.findMany({ where: { startedAt: { gte: cutoff } } });
    const uniqueVisitors = new Set(sessions.map((s: { visitorId: string | null }) => s.visitorId)).size;
    const totalSessions = sessions.length;
    const avgDuration = totalSessions > 0 ? Math.round(sessions.reduce((a: number, s: { durationSeconds: number }) => a + s.durationSeconds, 0) / totalSessions) : 0;
    const bounceRate = totalSessions > 0 ? Math.round((sessions.filter((s: { pageViews: number }) => s.pageViews === 1).length / totalSessions) * 100) : 0;
    ...
    pageViewEvents.forEach((e: { pageUrl: string | null }) => { if (e.pageUrl) pageViewsByUrl[e.pageUrl] = (pageViewsByUrl[e.pageUrl] || 0) + 1; });
    ...
    return {
      ...
      page_views_by_url: pageViewsByUrl,
      period: period || '30d',
    };
  }
  ...
  async restoreEvent(id: string) {
    ...
    if (!event) throw new NotFoundException('Analytics event not found');
    return event;
  }
