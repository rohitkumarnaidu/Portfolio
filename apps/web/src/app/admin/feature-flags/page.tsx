...
export default function FeatureFlagsPage() {
  ...
  const handleDelete = async (f: FeatureFlag) => {
    await deleteMutation.mutateAsync(f.flagKey);
  };
  ...
  const handleToggle = async (f: FeatureFlag) => {
    try {
      ...
      addToast({
        variant: 'success',
        title: `${f.flagKey} ${f.isEnabled ? 'disabled' : 'enabled'}`,
      });
    } catch {
      addToast({ variant: 'error', title: 'Failed to toggle' });
    }
  };
    ...
    () => [
      {
        ...
        render: (f) => (
          <div>
            ...
            {f.description && (
              <span className="text-xs text-text-tertiary block">{f.description}</span>
            )}
          </div>
        ),
      },
      {
        ...
        render: (f) => (
          <button
            ...
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${f.isEnabled ? 'bg-accent-500' : 'bg-surface-tertiary'}`}
          >
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${f.isEnabled ? 'translate-x-6' : 'translate-x-1'}`}
            />
          </button>
        ),
      },
      {
        ...
        width: '80px',
        sortable: true,
        render: (f) => <span className="text-xs text-text-secondary">{f.rolloutPercentage}%</span>,
      },
    ],
        ...
        <Card variant="elevated" className="mb-8">
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            <h3 className="font-display text-h4 text-text-primary">
              {editingKey ? 'Edit Flag' : 'New Feature Flag'}
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Input
                label="Flag Key *"
                value={form.flagKey}
                onChange={(e) => setForm((f) => ({ ...f, flagKey: e.target.value }))}
                required
