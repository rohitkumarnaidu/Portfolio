...
function StatCard({ label, value, sub, icon }: { label: string; value: string | number; sub?: string; icon: React.ReactNode }) {
  return (
    <div className="rounded-2xl bg-surface-secondary border border-border-primary p-5">
      ...
      <p className="text-h3 font-display text-text-primary">{typeof value === 'number' ? value.toLocaleString() : value}</p>
...
function DailyViewsChart({ data }: { data: Array<{ date: string; views: number }> }) {
  if (!data.length) return <div className="text-sm text-text-tertiary text-center py-8">No data yet</div>;
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
      ...
      {[0, 0.25, 0.5, 0.75, 1].map((frac) => {
        ...
        return (
          <g key={frac}>
            <line x1={padding.left} y1={y} x2={width - padding.right} y2={y} stroke="var(--border-primary)" strokeWidth="1" />
            <text x={padding.left - 8} y={y + 4} textAnchor="end" className="fill-text-tertiary text-[10px]" fontSize="10">
      ...
      {/* Dots */}
      {points.filter((_, i) => data.length > 10 ? i % Math.ceil(data.length / 8) === 0 || i === data.length - 1 : true).map((p, i) => (
        <circle key={i} cx={p.x} cy={p.y} r="3" fill="var(--accent-500)" stroke="var(--surface-secondary)" strokeWidth="2" />
      ))}

      {/* X-axis labels */}
      {points.filter((_, i) => data.length > 10 ? i % Math.ceil(data.length / 6) === 0 || i === data.length - 1 : true).map((p, i) => (
        <text
          key={i}
          x={p.x}
          y={height - 6}
...
function DonutChart({ data }: { data: { desktop: number; mobile: number; tablet: number } }) {
  ...
  const segments = [
    { label: 'Desktop', value: data.desktop, color: 'var(--accent-500)', pct: data.desktop / total },
  ...
  return (
    <div className="flex items-center gap-6">
      <svg viewBox="0 0 160 160" className="w-32 h-32 shrink-0">
        <circle cx="80" cy="80" r={radius} fill="none" stroke="var(--border-primary)" strokeWidth="20" />
        {segments.map((seg) => {
        ...
        <text x="80" y="76" textAnchor="middle" className="fill-text-primary text-xl font-display font-bold" fontSize="20">
          {Math.round(data.desktop * 100)}%
        </text>
        <text x="80" y="92" textAnchor="middle" className="fill-text-tertiary text-[10px]" fontSize="10">
          Desktop
        </text>
      </svg>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="h-28 rounded-2xl bg-surface-secondary border border-border-primary animate-pulse" />
          ))}
        </div>
  ...
  if (!summary) {
    return (
      <Card variant="elevated" padding="md" className="text-center py-12">
        <p className="text-text-secondary text-sm">No analytics data available yet. Start tracking by visiting the portfolio site!</p>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {loading && (
          <svg className="animate-spin w-4 h-4 text-accent-500 ml-2" fill="none" viewBox="0 0 24 24">
                  ...
                  <div key={page.path} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs text-text-tertiary font-mono ml-2">{page.views}</span>
                      </div>
                      <div className="h-1.5 rounded-full bg-surface-elevated overflow-hidden">
                        <div className="h-full rounded-full bg-accent-500/60 transition-all" style={{ width: `${barWidth}%` }} />
                      </div>
                    </div>
                  </div>
                );
              ...
              {summary.top_sources.map((src, i) => {
                ...
                return (
                  <div key={src.source} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs text-text-secondary capitalize truncate">{src.source}</span>
                        <span className="text-xs text-text-tertiary font-mono ml-2">{src.visitors}</span>
                      </div>
                      <div className="h-1.5 rounded-full bg-surface-elevated overflow-hidden">
                        <div className="h-full rounded-full bg-accent-500/60 transition-all" style={{ width: `${barWidth}%` }} />
                      </div>
