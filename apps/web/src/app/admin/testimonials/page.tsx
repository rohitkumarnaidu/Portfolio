...
export default function TestimonialsPage() {
  ...
  const resetForm = () => { setEditingTestimonial(null); setShowForm(false); };
  ...
  const handleSubmit = async (payload: Partial<Testimonial>) => {
    try {
      if (editingTestimonial) {
        await updateTestimonial.mutateAsync({ id: editingTestimonial.id, data: payload as unknown as Record<string, unknown> });
  ...
  const columns: Column<Testimonial>[] = useMemo(() => [
    {
      ...
      render: (t) => (
        <div className="flex items-center gap-2">
          ...
          {!t.is_visible && <span className="text-xs px-1.5 py-0.5 rounded bg-surface-elevated text-text-tertiary">Hidden</span>}
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      width: '80px',
      sortable: true,
      render: (t) => <span className="text-xs text-text-secondary">{'★'.repeat(t.rating)}{'☆'.repeat(5 - t.rating)}</span>,
    },
    {
      key: 'created_at',
      label: 'Date',
      width: '90px',
      sortable: true,
      render: (t) => <span className="text-xs text-text-tertiary">{new Date(t.created_at).toLocaleDateString()}</span>,
    },
  ], []);
    ...
    <div>
      ...
      <DataTable
        ...
        bulkActions={[
          {
            label: 'Bulk Delete',
            action: async (ids) => { await Promise.all(ids.map(id => deleteTestimonial.mutateAsync(id))); },
            variant: 'danger',
          ...
          {
            label: 'Bulk Verify',
            action: async (ids) => { await Promise.all(ids.map(id => updateTestimonial.mutateAsync({ id, data: { is_visible: true } }))); },
