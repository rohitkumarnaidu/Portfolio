...
export default function NotificationsPage() {
  ...
  const handleMarkRead = async (n: Notification) => {
    try {
      ...
      addToast({ variant: 'error', title: 'Failed to mark as read' });
    }
  };
  ...
  const handleMarkAllRead = async () => {
    try {
      ...
      addToast({ variant: 'error', title: 'Failed to mark all as read' });
    }
  };
    ...
    () => [
      {
        ...
        render: (n) => (
          <div className="flex items-center gap-2">
            ...
            <div>
              <span
                className={`text-sm ${n.isRead ? 'text-text-secondary' : 'font-medium text-text-primary'}`}
              >
                {n.title}
              </span>
              {n.body && (
                <span className="text-xs text-text-tertiary block truncate max-w-xs">{n.body}</span>
              )}
            </div>
          </div>
        ),
      },
      {
        ...
        width: '80px',
        sortable: true,
        render: (n) => <span className="text-xs text-text-secondary capitalize">{n.type}</span>,
      },
      {
        key: 'createdAt',
        label: 'Date',
