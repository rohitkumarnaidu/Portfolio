...
import type { UpdateLeadDto } from '../../modules/leads/dto';
...
export class AdminLeadsController {
  ...
  async findOne(@Param('id') id: string) {
    return { data: await this.leads.findById(id) };
  }
  ...
  async update(@Param('id') id: string, @Body() dto: UpdateLeadDto) {
    return { data: await this.leads.update(id, dto) };
  }
  ...
  async getNotes(@Param('id') id: string) {
    return { data: await this.leads.getNotes(id) };
  }
  ...
  async addNote(@Param('id') id: string, @Body('note') note: string) {
    return { data: await this.leads.addNote(id, note, 'admin') };
  }
  ...
  async restore(@Param('id') id: string) {
    return { data: await this.leads.restore(id) };
  }
  ...
  async hardDelete(@Param('id') id: string) {
    await this.leads.hardDelete(id);
  }
  ...
  async bulkDelete(@Body('ids') ids: string[]) {
    return this.leads.bulkDelete(ids);
  }
  ...
  async bulkUpdate(@Body('ids') ids: string[], @Body('data') data: Record<string, unknown>) {
    return { data: await this.leads.bulkUpdate(ids, data) };
  }
}
