...
- **Monitoring:**
  - `stats.js` during development for draw calls and FPS.
  - `navigator.hardwareConcurrency` and GPU tiering for progressive enhancement.
  - Level of Detail (LOD) for complex 3D models.
...
- **Browser caching:** Static assets (images, fonts, JS bundles) with 1-year TTL, content-addressable filenames.

## 8. Continuous Profiling & Monitoring

### 8.1 Tools
- **Sentry Performance:** Traces for every API request and page load. Monitors slow transactions.
- **Vercel Analytics:** Real User Monitoring for Core Web Vitals.
- **k6:** Scheduled load tests on staging environment (weekly).
- **Datadog (optional):** Infrastructure-level monitoring if self-hosted.
