...
### 3.2 Example Pattern
```typescript
// Test structure for any data-fetching hook
...
### 4.2 Data Flow
- **Page-level:** Renders a page with mocked API data. Verify that the data flows from page component → section component → card component.
...
- Portfolio navigation: Home → Projects → Detail, Blog → Post, Contact form submit.
...
- 3D canvases use relaxed thresholds (maxDiffPixels, maxDiffPixelRatio) due to WebGL non-determinism.
...
### 7.1 Automated (CI)
- **Unit level:** `jest-axe` runs on every component test. Verifies no auto-detected violations (missing labels, incorrect ARIA).
- **E2E level:** Playwright a11y scan on every page. Full page audit for WCAG 2.1 AA compliance.
- **Threshold:** Zero violations allowed in CI. Warnings are reviewed but not blocking.
