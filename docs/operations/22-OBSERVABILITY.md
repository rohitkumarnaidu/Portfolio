...
|-----------|-------------|
| `docs/operations/21-MONITORING.md` | Monitoring dashboards and alert rules |
| `docs/operations/DevOpsArchitecture.md` | DevOps pipelines and incident response |
| `docs/operations/DeploymentGuide.md` | Deployment observability integration |
| `docs/security/SecurityArchitecture.md` | Security monitoring and audit logging |
| `docs/operations/55-DISASTER-RECOVERY.md` | Recovery procedures during incidents |
| `docs/quality/PerformanceArchitecture.md` | Performance targets and benchmarks |
| `docs/operations/56-SLA-SLO.md` | Uptime SLAs and SLO definitions |
...
| `correlation_id` | string | Request correlation ID | UUID v4 or req_ prefix |
...
class Logger {
  ...
  error(message: string, error?: Error, data?: Record<string, unknown>) { this.log('error', message, data, error); }
  fatal(message: string, error?: Error, data?: Record<string, unknown>) { this.log('fatal', message, data, error); }
...
export class StructuredLogger implements LoggerService {
  ...
  log(message: string, context?: any) { this.logEntry('info', message, context); }
  warn(message: string, context?: any) { this.logEntry('warn', message, context); }
  error(message: string, error?: Error, context?: any) { this.logEntry('error', message, context, error); }
  debug(message: string, context?: any) { this.logEntry('debug', message, context); }
  verbose(message: string, context?: any) { this.logEntry('trace', message, context); }
}
```
...
|-------|---------|----------|
| **Incident declared** | Telegram | "🚨 *Incident Declared*: {title} | Severity: {tier} | Impact: {services} | Owner: {name}" |
| **Status update** | Incident channel | "🔄 *Update*: {phase} | {action_taken} | Time elapsed: {duration}" |
| **Resolution** | Telegram + Email | "✅ *Resolved*: {title} | Duration: {duration} | Root cause: {cause} | Post-mortem: {link}" |
| **Weekly digest** | Email | "📊 *Weekly Incident Report* | {count} incidents | {critical} critical | Top cause: {cause}" |
...
| Reference | Description |
|-----------|-------------|
| `docs/operations/21-MONITORING.md` (v5.0) | Enterprise Monitoring Architecture — alert rules, SLOs, runbooks triggered by observability signals (note: monitoring content consolidated into 22-OBSERVABILITY.md v5.0) |
| `docs/operations/AnalyticsArchitecture.md` (v5.0) | Enterprise Analytics Strategy — business-focused observability events, conversion funnels, dashboards |
| `docs/ai/17-AI_INSTRUCTIONS.md` (v5.0) | AI Operating Model — §19 AI Monitoring (10 alert rules), §20 Failure Recovery (procedures + runbook) |
...
| `docs/security/SecurityArchitecture.md` (v5.0) | Security Architecture — §29 Security Monitoring (security-specific observability, audit logs, WAF events) |
| `docs/database/DatabaseArchitecture.md` (v5.0) | Database Schema — §12 Analytics Tables (structured_logs table, ai_cost_log, audit_logs) |
| `docs/api/12-API.md` (v5.0) | API Documentation — correlation ID propagation, request logging, health check endpoints |
| `docs/quality/PerformanceArchitecture.md` (v3.0) | Performance Budgets — Web Vitals targets, API latency thresholds, optimization techniques |
| `docs/operations/DevOpsArchitecture.md` (v3.0) | DevOps — CI/CD pipeline observability, build metrics, deploy traces |
| `docs/operations/25-CICD.md` (v3.0) | CI/CD — pipeline configuration, quality gates, deployment health checks |
| `docs/security/16-COMPLIANCE.md` (v3.0) | Compliance — GDPR Art. 5 (data minimization), Art. 32 (security of processing), audit log retention |
| `docs/architecture/10-TECHSTACK.md` (v5.0) | Tech Stack — all observability tools listed with versions (Sentry, PostHog, Umami, Better Uptime) |
| `docs/MASTER-INDEX.md` (v3.0) | Master Index — document dependency graph, version history, cross-reference map |
| `docs/design/BackendArchitecture.md` | Backend Architecture — §14.4 Graceful Degradation, §18.6 Backup & Disaster Recovery |
| `docs/design/AdminArchitecture.md` | Admin Dashboard — §3 UI States, §4.5 Alert Configuration, §5 Notification System, §10 Health/SLA Dashboards |
