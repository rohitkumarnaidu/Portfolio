...
# Memory Architecture Ã¢â‚¬â€ Three-Tier Enterprise Memory System
...
> **AI Operating Model:** docs/08-ai/17-AI_INSTRUCTIONS.md | **Agent System:** docs/08-ai/18-AGENTS.md | **Implementation:** docs/08-ai/08h-AI-ASSISTANT-IMPLEMENTATION.md
...
3. [Tier 1 Ã¢â‚¬â€ Session Memory](#3-tier-1--session-memory)
4. [Tier 2 Ã¢â‚¬â€ Cache Memory](#4-tier-2--cache-memory)
5. [Tier 3 Ã¢â‚¬â€ Persistent Memory](#5-tier-3--persistent-memory)
...
graph TB
    subgraph "Three-Tier Memory Architecture"
        subgraph "Tier 1 Ã¢â‚¬â€ Session Memory"
            direction TB
        ...
        subgraph "Tier 2 Ã¢â‚¬â€ Cache Memory"
            direction TB
        ...
        subgraph "Tier 3 Ã¢â‚¬â€ Persistent Memory"
            direction TB
...
| Aspect          | Tier 1 Ã¢â‚¬â€ Session             | Tier 2 Ã¢â‚¬â€ Cache                          | Tier 3 Ã¢â‚¬â€ Persistent |
| --------------- | ---------------------------------- | --------------------------------------------- | ------------------------- |
...
## 3. Tier 1 Ã¢â‚¬â€ Session Memory

### 3.1 Conversation History

Session memory maintains the active conversation history for each chat session. The ConversationManager (`docs/08-ai/08h-AI-ASSISTANT-IMPLEMENTATION.md` Ã‚Â§6.2) manages this entirely in-memory using a dictionary of `Conversation` dataclass instances.
...
## 5. Tier 3 Ã¢â‚¬â€ Persistent Memory
...
| ------------------------- | ------------------------ | ---------------------------------------------------- |
| Tier 1 Ã¢â‚¬â€ Session    | In-memory isolation      | Sessions stored in scoped dict, no shared state      |
| Tier 1 Ã¢â‚¬â€ Session    | Session ID validation    | UUID format validated before use                     |
| Tier 2 Ã¢â‚¬â€ Cache      | Key-based access         | Cache accessed by deterministic hash only            |
| Tier 2 Ã¢â‚¬â€ Cache      | TTL-based cleanup        | Automatic eviction prevents stale data accumulation  |
| Tier 3 Ã¢â‚¬â€ Persistent | Row-Level Security (RLS) | PostgreSQL RLS policies on all chat tables           |
| Tier 3 Ã¢â‚¬â€ Persistent | Encryption at rest       | Supabase provides AES-256 encryption                 |
| Tier 3 Ã¢â‚¬â€ Persistent | CASCADE deletes          | Message deletion cascades from conversation deletion |
...
The `ConversationManager` (defined in `docs/08-ai/08h-AI-ASSISTANT-IMPLEMENTATION.md` Ã‚Â§6.2) is the primary memory provider for all agents:
...
| ----------------------------------------------- | ------------------------------------ | --------------------------------------------------------------------------- |
| `docs/08-ai/08h-AI-ASSISTANT-IMPLEMENTATION.md` | Memory implementation blueprint      | SS6.2 ConversationManager, SS2.2 Cache Service, SS2.3 Embedding Cache Table |
| `docs/08-ai/18-AGENTS.md`                       | Agent memory architecture definition | SS17 Agent Memory Architecture, SS17.2 Per-Agent Memory Configuration       |
| `docs/08-ai/08g-AI-ASSISTANT-ARCHITECTURE.md`   | Memory strategy overview             | SS6 Memory Strategy, SS6.1 Three-Tier Architecture, SS6.2-6.6               |
| `docs/10-api/49-CACHE-ARCHITECTURE.md`          | Broader caching strategy (CDN, ISR)  | SS2 Cache Layers, SS3 CDN Layer, SS4 Application Caching                    |
| `docs/05-architecture/SystemArchitecture.md`    | System architecture context          | SS7 AI Architecture, SS4 Data Architecture                                  |
| `docs/08-ai/AGENT.md`                           | Agent base class definition          | Agent anatomy, memory configuration                                         |
| `docs/08-ai/AGENT-REGISTRY.md`                  | Agent registration and discovery     | Agent capability manifests, routing                                         |
| `docs/09-database/DatabaseArchitecture.md`      | Database schema and indexes          | Chat tables, RLS policies, indexing                                         |
| `docs/11-security/SecurityArchitecture.md`      | Security policies and controls       | Encryption, access control, PII protection                                  |
| `docs/11-security/16-COMPLIANCE.md`             | Compliance framework                 | GDPR, data retention, right to deletion                                     |
| `docs/35-quality/PerformanceArchitecture.md`    | Performance targets and monitoring   | Query latency, throughput, SLA definitions                                  |
| `docs/21-operations/58-COST-MANAGEMENT.md`      | Cost tracking and optimization       | API costs, database costs, budget allocation                                |
| `docs/08-ai/17-AI_INSTRUCTIONS.md`              | AI operating model                   | SS9 Memory Rules (MEM-001 through MEM-008)                                  |
| `docs/21-operations/22-OBSERVABILITY.md`        | Monitoring and observability         | Metrics collection, alerting, dashboards                                    |
| `docs/10-api/47-BACKGROUND-JOBS.md`             | Cron job specifications              | Retention cleanup, cache invalidation                                       |
...
| ----------- | --------------------------------------------------------------------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | ------------ |
| MEM-DEC-001 | Three-tier memory architecture (session, cache, persistent)           | Memory system design      | Clear separation of concerns: session for ephemeral conversation state, cache for performance optimization, persistent for audit and recovery            | Two-tier (session + persistent only) Ã¢â‚¬â€ no caching layer degrades performance; Unified single store Ã¢â‚¬â€ no isolation, mixing concerns | Jun 2026      | Dec 2026     |
...
_Document Version: 1.0 -- Enterprise-Grade Three-Tier Memory Architecture_
