...
export function Contact({ data }: { data?: Section }) {
  ...
  return (
    <SectionWrapper
      ...
      subtitle={content.subtitle || "Have a project in mind? I'd love to hear about it. Send me a message and I'll get back to you within 24 hours."}
      className="max-w-4xl"
      animate={false}
    >
      <div ref={ref}>
        <div className={`grid grid-cols-1 lg:grid-cols-5 gap-8 transition-all duration-700 delay-100 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          {/* Contact Info */}
            ...
            <div className="glass-subtle rounded-2xl p-6 space-y-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <a href={`mailto:${content.email || 'hello@portfolio.com'}`} className="text-body-sm text-text-secondary hover:text-accent-500 transition-colors">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-body-sm text-text-secondary">{content.location || 'Remote / Worldwide'}</p>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-3">
            <form onSubmit={handleSubmit} className="glass-subtle rounded-2xl p-6 md:p-8 space-y-5" noValidate>
              ...
              <div className="space-y-1.5">
                ...
                />
                {errors.message && (
                  <p className="text-sm text-semantic-error" role="alert">{errors.message}</p>
                )}
              </div>

              {status === 'success' && (
                <div className="flex items-center gap-2 p-3 rounded-xl bg-semantic-success-bg border border-semantic-success/20">
                  <svg className="w-5 h-5 text-semantic-success shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
                  <p className="text-sm text-semantic-success">Message sent! I&apos;ll get back to you within 24 hours.</p>
                </div>
              )}

              {status === 'error' && (
                <div className="flex items-center gap-2 p-3 rounded-xl bg-semantic-error-bg border border-semantic-error/20">
                  <svg className="w-5 h-5 text-semantic-error shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <p className="text-sm text-semantic-error">Something went wrong. Please try again later.</p>
                </div>
              )}
