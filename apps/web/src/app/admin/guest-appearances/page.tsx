...
import {
  useAdminGuestAppearances,
  useCreateGuestAppearance,
  useUpdateGuestAppearance,
  useDeleteGuestAppearance,
  useBulkDeleteGuestAppearances,
} from '@/lib/hooks/useGuestAppearances';
...
export default function GuestAppearancesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const payload: Partial<GuestAppearance> = {
        title: form.title,
        host: form.host,
        url: form.url || undefined,
        description: form.description || undefined,
        type: form.type as GuestAppearance['type'],
        date: form.appearanceDate || undefined,
      };
      if (editingId) {
  ...
  const handleDelete = async (a: GuestAppearance) => {
    ...
    () => [
      {
        ...
        sortable: true,
        grow: true,
        render: (a) => <span className="text-sm font-medium text-text-primary">{a.title}</span>,
      },
      {
        key: 'type',
        label: 'Type',
        width: '80px',
        sortable: true,
        render: (a) => <span className="text-xs text-text-secondary capitalize">{a.type}</span>,
      },
      {
        key: 'host',
        label: 'Host',
        sortable: true,
        width: '120px',
        render: (a) => <span className="text-xs text-text-secondary">{a.host}</span>,
      },
      {
        key: 'date',
        label: 'Date',
        ...
        sortable: true,
        render: (a) =>
          a.date ? <span className="text-xs text-text-tertiary">{a.date}</span> : null,
      },
    ],
    [],
  );
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-body-sm text-text-secondary mt-1">
            Manage podcasts, talks, interviews, and webinars
          </p>
        </div>
        <Button
          onClick={() => {
