  ...
  const columns: Column<Experience>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        width: '80px',
        sortable: true,
        render: (exp) => <span className="text-xs text-text-tertiary">{exp.startDate}</span>,
      },
      {
        key: 'endDate',
        label: 'End',
        ...
        sortable: true,
        render: (exp) => (
          <span className="text-xs text-text-tertiary">{exp.endDate || 'Present'}</span>
        ),
      },
    ],
    [],
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        {!showForm && <Button onClick={() => setShowForm(true)}>Add Experience</Button>}
      </div>
        ...
        bulkActions={[
          {
            label: 'Bulk Delete',
            action: async (ids) => {
              await Promise.all(ids.map((id) => deleteExperience.mutateAsync(id)));
            },
            variant: 'danger',
          ...
          },
          {
            ...
            action: async (ids) => {
              await Promise.all(
                ids.map((id) => updateExperience.mutateAsync({ id, data: { isVisible: true } })),
              );
            },
          ...
          },
          {
            ...
            action: async (ids) => {
              await Promise.all(
                ids.map((id) => updateExperience.mutateAsync({ id, data: { isVisible: false } })),
              );
            },
