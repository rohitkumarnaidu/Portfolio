...
This document defines detailed user journey maps for each primary persona defined in `UserPersonas.md`. Unlike the swimlane flows in `UserFlows.md` (which focus on step-by-step system interactions), these journey maps model the **full emotional and behavioral experience** — from awareness through post-conversion — highlighting pain points, opportunities, and success metrics at every phase.

### 1.1 Journey Map Structure

Each journey map includes:

1. **Persona context** — Who, what, why
2. **Journey phases** — End-to-end lifecycle stages
3. **Touchpoints** — Where users interact with the system
4. **Emotional journey** — Highs, lows, and inflection points
5. **Pain points & opportunities** — Friction and improvement areas
6. **Success metrics per phase** — Quantitative targets
...
**Persona:** Sarah Chen — Senior Technical Recruiter
...
| **Emotional State** | 😐 Neutral — "Another portfolio to review" | 😊 Hopeful — "This loaded fast" | 😊 Interested — "Clear, concise info" | 🤔 Evaluating — "Skills match but need proof" | 😊 Confident — "Evidence supports claims" | 🎯 Satisfied — "Got what I needed" | ☺️ Positive — "Good candidate, easy process" |
...
| **Drop-off Risk** | ⚠️ High if not in search results | ⚠️ High if >3s load | ⚠️ Medium if hero is vague | ⚠️ High if skills don't match | ⚠️ Medium if verification fails | ⚠️ Low (sunk cost) | N/A |
...
- **Landing**: Fast load → positive start; slow load → immediate negative
- **Skills → Projects transition**: Biggest drop-off risk if skills don't match
- **Verification**: Second peak — testimonials and GitHub build confidence
- **Action**: Must be frictionless to convert the built-up positive emotion

### 2.3 Key Improvements for This Journey

1. **Sub-1.5s LCP**: Above-the-fold hero must render instantly — static HTML, no client JS dependency
...
4. **Resume access**: Direct PDF link in header or hero — no login, no modal
5. **Performance budget**: Enforce via CI Lighthouse gates — fail build if LCP > 1.5s

---

## 3. Journey 2: Client Assessment Journey

**Persona:** Marcus Johnson — CTO / Startup Founder
...
| **Emotional State** | 😊 Curious — "Need to find the right person" | 😐 Cautious — "Does this person fit my needs?" | 🤔 Analytical — "Digging into the details" | 😊 Warming — "Others trust them, that's good" | 😊 Confident — "Ready to reach out" | 🎯 Hopeful — "Looking forward to discussion" |
...
| **Opportunities** | Niche SEO strategy (e.g., "NestJS consultant") | Clear services + engagement model in hero/secondary nav | Case study template: Problem → Approach → Result (with metrics) | Video testimonials, client logos, case study PDFs | Smart form (adapts fields based on project type), calendar scheduling | Auto-reply within 60s, SLA guarantee |
...
| **Drop-off Risk** | ⚠️ Low (specific need) | ⚠️ Medium if About is generic | ⚠️ High if no quantified results | ⚠️ Medium if social proof missing | ⚠️ Medium if form is long | ⚠️ High if response is slow |
...
😐 Neutral┤──────────╱╲──╱╱          ╲╱          ╲╲_____
         │         ╱  ╲╱                                 
😟 Frustrated┤      Qualify  Eval   Trust  Convert       
         │  Disc.    |       |       |       |   
😡 Angry  ┤─────────────────────────────────────────
                 Time →
```
...
Key inflection points:
- **Qualification → Evaluation**: High risk if services are unclear — user leaves before seeing evidence
- **Trust Building**: Testimonials are the conversion bridge — must be authentic and relevant
- **Conversion**: Form length inversely correlates with completion rate — optimize for 4-5 fields max
...
2. **Case study template**: Every case study follows Problem → Approach → Result format with hard metrics
...
**Persona:** Alex Rivera — Portfolio Owner
...
| **Emotional State** | 😊 Routine — "Part of my workflow" | 😊 Informed — "Good, 3 new leads" | 😊 Productive — "Following up with leads" | 😊 Creative — "Sharing new work" | 🤔 Analytical — "What's resonating?" | ☺️ Done — "Everything is current" |
...
| **Drop-off Risk** | ⚠️ Low (motivated user) | ⚠️ Low | ⚠️ Medium (notification fatigue) | ⚠️ Medium (frustration with editor) | ⚠️ Low | N/A |

### 4.2 Key Improvements for This Journey

1. **Actionable dashboard**: Widgets prioritized by urgency — unanswered leads first, then content freshness warnings
2. **CMS reliability**: Autosave every 30 seconds, revision history with diff view, preview on mobile/desktop/tablet
3. **Lead management pipeline**: Inbox (new) → Read → Responded → Closed, with email integration
...
**Persona:** Jordan Kim — Developer / OSS Contributor
...
| **Emotional State** | 😊 Excited — "This looks promising!" | 😐 Cautious — "Is this well-maintained?" | 😊 Hopeful — "Setup is smooth" | 😊 Engaged — "I understand the structure" | 😊 Productive — "I can contribute!" | 😐 Anxious → 😊 Proud — "Waiting for review" |
...
| **Opportunities** | Active badges, recent commit visible | Setup → contribution path in < 1 hour | Docker Compose with one command | Good first issues with implementation hints | PR template with checklist, auto-assign reviewers | CI under 5 min, SLA on PR review < 48h |
...
| **Drop-off Risk** | ⚠️ Medium (many repos compete) | ⚠️ High (poor docs = instant abandon) | ⚠️ High (setup friction) | ⚠️ Medium (complexity) | ⚠️ Low (invested) | ⚠️ Medium (slow review) |
...
**Persona:** Priya Sharma — Senior Engineer / Blog Reader
...
| **Emotional State** | 😊 Curious — "Interesting title" | 😊 Engaged — "Great explanation!" | 😊 Impressed — "They really know their stuff" | 😊 Generous — "Sharing this with my network" | 😊 Loyal — "I'll keep coming back" |
...
| **Drop-off Risk** | ⚠️ Medium (distraction from social) | ⚠️ High (shallow content) | ⚠️ Medium (inconsistency) | ⚠️ Low | ⚠️ Medium (content drought) |
...
|-------|-----------|--------|-------|-------------|------|
| **Entry (0–5s)** | ✅ Fast load critical | ✅ Fast load critical | ✅ Login speed | ✅ Repo load | ✅ Page load |
| **Hero/Core (5–15s)** | ✅ Role + availability | ✅ Services + credibility | ✅ Dashboard KPIs | ✅ README quality | ✅ Content quality |
...
| Content staleness | All visitors | Admin CMS → automatic freshness updates |
...
| **P0** | Case study template (Problem→Approach→Result) | Client | Medium | High (trust + conversion) |
...
- **Admin journey**: Time trial — measure content update speed from login to publish
...
*Document Version: 1.0 — User Journey Maps*
