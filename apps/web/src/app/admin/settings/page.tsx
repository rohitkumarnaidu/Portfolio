...
export default function SettingsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!key.trim()) {
        addToast({ variant: 'error', title: 'Setting key is required' });
        return;
      }
      ...
      addToast({ variant: 'error', title: 'Failed to save setting' });
    }
  };
  ...
  const handleInlineSave = async (key: string, value: string) => {
    try {
      await upsertSetting.mutateAsync({
        key,
        data: { settingValue: value } as Partial<SystemSetting>,
      });
      ...
      addToast({ variant: 'error', title: 'Failed to update setting' });
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      addToast({ variant: 'error', title: 'Failed to delete setting' });
    }
  };
    ...
    return (
      <div className="space-y-4 animate-pulse">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="h-32 rounded-xl bg-surface-secondary border border-border-primary"
          />
        ))}
      </div>
    );
    ...
    <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, settingGroup: e.target.value }))}
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      settingType: e.target.value as SettingForm['settingType'],
                    }))
                  }
                  className="w-full rounded-xl border border-border-primary bg-surface-secondary px-4 py-2.5 text-sm text-text-primary"
                >
                ...
                {items.length === 0 ? (
                  ...
                  items.map((setting) => {
                    ...
                    const currentValue = isEditing
                      ? (editingValues[setting.settingKey] ?? '')
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs px-1.5 py-0.5 rounded bg-surface-elevated text-text-tertiary font-mono">
                              {setting.settingType}
                            </span>
                          </div>
                          {setting.description && (
                          ...
                          <div className="mt-2 max-w-md">
                            {isEditing ? (
                              <div className="flex gap-2">
                                ...
                                <button
                                  onClick={() =>
                                    handleInlineSave(String(setting.settingKey), currentValue)
                                  }
                                  className="px-3 py-1.5 text-xs font-medium rounded-lg bg-accent-500 text-white hover:bg-accent-600 transition-colors"
                                >
                                  Save
                                </button>
                                <button
                                  onClick={() =>
                                    setEditingValues((prev) => {
                                      const n = { ...prev };
                                      delete n[String(setting.settingKey)];
                                      return n;
                                    })
                                  }
                                  className="px-3 py-1.5 text-xs font-medium rounded-lg bg-surface-elevated text-text-secondary hover:text-text-primary transition-colors"
                                >
                                  Cancel
                                </button>
                              </div>
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          ...
                          <button
                            onClick={() => setDeleteTarget(setting.settingKey ?? null)}
      ...
      >
        <p className="text-sm text-text-secondary">
          Are you sure you want to delete setting &ldquo;{deleteTarget}&rdquo;?
        </p>
