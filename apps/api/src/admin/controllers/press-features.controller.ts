...
import type {
  CreatePressFeatureDto,
  UpdatePressFeatureDto,
} from '../../modules/press-features/dto';
...
export class AdminPressFeaturesController {
  ...
  @Get(':id')
  @Roles('admin', 'editor', 'viewer')
  @ApiOperation({ summary: 'Get press feature by ID' })
  async findOne(@Param('id') id: string) {
    return { data: await this.service.findById(id) };
  }
  @Post()
  @Roles('admin', 'editor')
  @Audit({ action: 'create', resource: 'press_feature' })
  @ApiOperation({ summary: 'Create a press feature' })
  async create(@Body() dto: CreatePressFeatureDto) {
    return { data: await this.service.create(dto) };
  }
  @Patch(':id')
  @Roles('admin', 'editor')
  @Audit({ action: 'update', resource: 'press_feature' })
  @ApiOperation({ summary: 'Update a press feature' })
  async update(@Param('id') id: string, @Body() dto: UpdatePressFeatureDto) {
    return { data: await this.service.update(id, dto) };
  }
  @Delete(':id')
  @Roles('admin')
  @Audit({ action: 'delete', resource: 'press_feature' })
  @HttpCode(HttpStatus.NO_CONTENT)
  @ApiOperation({ summary: 'Delete a press feature' })
