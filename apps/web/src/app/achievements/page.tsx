    ...
    <div className="min-h-dvh bg-surface-primary">
      <div className="max-w-6xl mx-auto px-4 py-16 md:py-24">
        <div className="mb-12">
          ...
          <p className="text-body text-text-secondary max-w-2xl">Certifications, awards, and professional recognitions.</p>
        </div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1,2,3,4,5,6].map(i => (
              <div key={i} className="rounded-2xl bg-surface-secondary border border-border-primary p-6 animate-pulse h-48" />
            ))}
          </div>
        ) : !achievements?.length ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {achievements.map(a => (
              <div key={a.id} className="rounded-2xl bg-surface-secondary border border-border-primary p-6 hover:border-border-accent transition-colors group">
                <div className="flex items-start gap-4">
                  {a.badgeImageUrl ? (
                    <img src={a.badgeImageUrl} alt="" className="w-12 h-12 rounded-xl object-contain bg-surface-elevated shrink-0" />
                  ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-display text-h4 text-text-primary group-hover:text-accent-500 transition-colors">{a.title}</h3>
                    <p className="text-sm text-text-secondary mt-1">{a.issuer}</p>
                    {a.category && <span className="inline-block text-xs px-2 py-0.5 rounded bg-surface-elevated text-text-tertiary mt-2 capitalize">{a.category}</span>}
                  </div>
                </div>
                {a.description && <p className="text-sm text-text-tertiary mt-3">{a.description}</p>}
                <div className="flex items-center gap-3 mt-4 text-xs text-text-tertiary">
                  {a.achievedDate && <span>{a.achievedDate}</span>}
                  {a.credentialUrl && (
                    <a href={a.credentialUrl} target="_blank" rel="noopener noreferrer" className="text-accent-500 hover:underline ml-auto">
                      View credential
                    </a>
                  )}
                </div>
