...
**Practice:** The `@portfolio/shared` workspace (`packages/shared/`) defines all Zod schemas and TypeScript interfaces. Both `apps/web` and `apps/api` import from this single source of truth. The `request<T>()` wrapper in `apps/web/src/lib/api.ts:39` returns strongly typed data via the `ApiResponse<T>` envelope (line 23-26). Zod schemas power validation on both sides — the global `ValidationPipe` at `apps/api/src/main.ts:49-56` validates incoming DTOs with `whitelist: true, forbidNonWhitelisted: true`.
...
**Auth Level:** Admin controllers are guarded by `@UseGuards(JwtAuthGuard, RolesGuard)` — every route explicitly declares required roles (`apps/api/src/admin/controllers/*.ts`). JWT tokens are never stored in cookies (only `localStorage` on the client, attached via `Authorization` header at `apps/web/src/lib/api.ts:53-55`).
...
- Documentation is reviewed alongside code in PRs — changes to the architecture must update the relevant docs
...
- `apps/api/src/main.ts` — bootstrap (Helmet, CORS, Pipes, Swagger)
- `apps/api/src/common/filters/global-exception.filter.ts` — error envelope
- `apps/api/src/portfolio/portfolio.module.ts` — module registrations
- `apps/api/src/admin/admin.module.ts` — admin module registrations
...
**Form Submission:** Contact forms use progressive enhancement — they work as standard POST forms without JavaScript. When JS loads, they upgrade to AJAX with React Hook Form validation.
