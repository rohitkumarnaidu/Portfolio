      ...
      .filter(
        (p) =>
          ...
          (p.category === project.category ||
            p.tech_stack?.some((t) => project.tech_stack?.includes(t))),
      )
      .slice(0, 3);
  }, [project, allProjects]);
    ...
    <article className="pt-32 pb-24">
      <div className="section-container mb-8">
        ...
        <div className="max-w-4xl">
          <div
            ...
            role="list"
            aria-label="Technologies and category"
          >
      ...
      <div className="w-full aspect-[21/9] max-h-[600px] bg-surface-elevated mb-16 flex items-center justify-center border-y border-border-primary relative overflow-hidden">
        {project.cover_image ? (
          ...
          <span className="text-text-tertiary font-display text-h2" aria-hidden="true">
      ...
      <div className="section-container max-w-3xl">
        ...
        Object.keys(project.content).length > 0 ? (
          <div
            className="prose prose-zinc dark:prose-invert max-w-none space-y-8"
            role="article"
          ...
          >
            {Object.entries(project.content).map(
              ([key, value]) =>
                typeof value === 'string' && (
                  <section key={key}>
                    <h2 className="text-h2 font-display text-text-primary mb-4 capitalize">
                      {key.replace(/_/g, ' ')}
                    </h2>
                    <div
            ...
            <section>
              ...
              <div className="flex flex-wrap gap-2" role="list" aria-label="Technologies used">
                {project.tech_stack?.map((tech) => (
                  <span
                    key={tech}
                    className="px-3 py-1.5 text-sm font-mono rounded-lg bg-surface-elevated text-text-secondary border border-border-primary"
                    role="listitem"
                  >
                    {tech}
                  </span>
        ...
        {project.metrics &&
          ...
          Object.keys(project.metrics).length > 0 && (
            ...
            >
              <h2 id="metrics-heading" className="text-h2 font-display text-text-primary mb-6">
                Metrics &amp; Impact
              </h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4" role="list">
                {Object.entries(project.metrics).map(([key, value]) => (
                  <div
                    key={key}
                    className="p-4 rounded-xl bg-surface-secondary border border-border-primary text-center"
                    role="listitem"
                  >
        ...
        {relatedProjects.length > 0 && (
          ...
          >
            <h2
              ...
              className="text-h2 font-display text-text-primary mb-6"
            >
              Related Projects
            </h2>
                  ...
                  <div className="p-4 rounded-xl bg-surface-secondary border border-border-primary hover:border-border-accent transition-all">
                    ...
                    >
                      {rp.tech_stack?.slice(0, 2).map((t) => (
                        <span
                          key={t}
                          className="text-xs px-2 py-0.5 rounded bg-surface-elevated text-text-tertiary"
                        >
                          {t}
                        </span>
                      ))}
