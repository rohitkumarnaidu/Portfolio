...
export default function MediaPage() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      addToast({ variant: 'error', title: 'Failed to delete asset' });
    }
  };
      ...
      {isLoading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {Array.from({ length: 8 }).map((_, i) => (
            <div
              key={i}
              className="aspect-square rounded-xl bg-surface-secondary border border-border-primary animate-pulse"
            />
          ))}
        </div>
      ) : !assets?.length ? (
        <div className="text-center py-16">
          <p className="text-body text-text-secondary">
            No media assets yet. Upload your first file above.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
        ...
        {viewAsset && (
          <div className="space-y-4">
            {isImage(viewAsset.mimeType) ? (
              <img
                src={viewAsset.filePath}
                alt={viewAsset.altText || ''}
                className="max-h-96 w-full object-contain rounded-xl bg-surface-elevated"
              />
            ) : (
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <span className="text-text-primary">{viewAsset.mimeType}</span>
              </div>
              <div>
                <span className="text-text-tertiary">Size:</span>{' '}
                <span className="text-text-primary">{formatSize(viewAsset.fileSizeBytes)}</span>
              </div>
              {viewAsset.width && (
                <div>
                  <span className="text-text-tertiary">Dimensions:</span>{' '}
                  <span className="text-text-primary">
                    {viewAsset.width} x {viewAsset.height}
                  </span>
                </div>
              )}
              {viewAsset.altText && (
                <div className="col-span-2">
                  <span className="text-text-tertiary">Alt Text:</span>{' '}
                  <span className="text-text-primary">{viewAsset.altText}</span>
                </div>
              )}
            </div>
            <div className="flex gap-2">
              <Button
                variant="secondary"
                onClick={() => {
                  navigator.clipboard.writeText(viewAsset.filePath);
                  addToast({ variant: 'success', title: 'URL copied' });
                }}
              >
                Copy URL
              </Button>
