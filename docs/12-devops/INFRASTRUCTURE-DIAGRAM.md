...
1-3 same as public flow 4. Auth: JWT token verified by Passport.js guards (JwtAuthGuard) 5. RBAC: Role-based access enforced by RolesGuard (@Roles decorator) 6. Audit: All mutations logged via @Audit decorator 7. Cache: Admin routes bypass CDN cache; server-side caching via @CacheTTL
