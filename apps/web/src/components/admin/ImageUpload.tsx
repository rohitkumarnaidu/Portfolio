...
export function ImageUpload({ value, onChange, error, label = 'Upload Image' }: ImageUploadProps) {
  ...
  return (
    <div className="w-full space-y-1.5">
      ...
      <div
        className={`flex flex-col items-center justify-center border-2 border-dashed rounded-lg p-6 transition-colors ${error ? 'border-red-500 bg-red-500/5' : 'border-border-primary hover:border-accent-500 bg-surface-secondary'}`}
      >
        {value ? (
          <div className="relative w-full aspect-video rounded overflow-hidden mb-4 group">
            ...
            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
              ...
              <Button size="sm" variant="danger" onClick={() => onChange('')} type="button">
                Remove
              </Button>
            </div>
          </div>
