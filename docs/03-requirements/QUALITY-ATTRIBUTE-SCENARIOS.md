# Quality Attribute Scenarios Ã¢â‚¬â€ Portfolio Platform
...
> **Source** (who/what initiates) Ã¢â€ â€™ **Stimulus** (what happens) Ã¢â€ â€™ **Artifact** (which system component) Ã¢â€ â€™ **Environment** (under what conditions) Ã¢â€ â€™ **Response** (how the system reacts) Ã¢â€ â€™ **Response Measure** (how we verify)

---

## Scenario 1: Performance Ã¢â‚¬â€ Concurrent API Requests
...
| **Artifact** | Vercel CDN Ã¢â€ â€™ Next.js ISR cache Ã¢â€ â€™ NestJS API (on cache miss) |
...
| **Current Status** | Ã°Å¸â€Â´ Not tested Ã¢â‚¬â€ system not deployed |

---

## Scenario 2: Availability Ã¢â‚¬â€ Database Failover
...
| **Artifact** | NestJS API Ã¢â€ â€™ PrismaService Ã¢â€ â€™ Supabase PostgreSQL connection pool |
...
| **Current Status** | Ã°Å¸â€Â´ Not tested Ã¢â‚¬â€ architecture supports ISR cache resilience but failover behavior not verified |

---

## Scenario 3: Security Ã¢â‚¬â€ Unauthorized Access Attempt
...
| **Artifact** | NestJS API Gateway Ã¢â€ â€™ JwtAuthGuard on `AdminModule` Ã¢â€ â€™ RoutesGuard Ã¢â€ â€™ controller |
...
| **Current Status** | Ã°Å¸Å¸Â¢ Designed and guard-implemented Ã¢â‚¬â€ `JwtAuthGuard` applied globally to `AdminModule`. Verification pending via integration tests. |

---

## Scenario 4: Modifiability Ã¢â‚¬â€ Adding a New Entity Type
...
| **Current Status** | Ã°Å¸Å¸Â¢ Pattern established Ã¢â‚¬â€ three scaffolded entity examples exist (Sections, Projects, Skills) confirming the pattern works |

---

## Scenario 5: Usability Ã¢â‚¬â€ First-Time Admin Navigation
...
| **Artifact** | Admin SPA (Next.js client-side render) Ã¢â€ â€™ TanStack React Query Ã¢â€ â€™ NestJS API |
...
| **Verification Method** | Task-based usability test with 3 participants who have never seen the admin panel. Track time-on-task, error rate, and System Usability Scale (SUS) score. Target SUS Ã¢â€°Â¥ 80. |
| **Current Status** | Ã°Å¸â€Â´ Not testable Ã¢â‚¬â€ admin dashboard not yet implemented |

---

## Scenario 6: Scalability Ã¢â‚¬â€ Traffic Spike
...
| **Artifact** | Vercel CDN Ã¢â€ â€™ Next.js ISR Ã¢â€ â€™ NestJS API Ã¢â€ â€™ Supabase PostgreSQL. AI service (FastAPI) receives 100 concurrent chat requests. |
...
| **Current Status** | Ã°Å¸â€Â´ Not tested Ã¢â‚¬â€ load testing infrastructure not configured |

---

## Scenario 7: Testability Ã¢â‚¬â€ Running the Test Suite
...
| **Response** | API unit tests complete within 30 seconds verifying `LeadsService.create()`, `LeadsService.findAll()`, `LeadsService.exportCSV()`. Integration tests complete within 2 minutes verifying the full request/response cycle through controller Ã¢â€ â€™ guard Ã¢â€ â€™ service Ã¢â€ â€™ Prisma mock. E2E tests complete within 5 minutes via Playwright against a local Next.js dev server. Coverage report shows Ã¢â€°Â¥ 80% line coverage for `LeadsModule`. |
| **Response Measure** | Full test suite < 10 minutes. Unit tests < 30s. Integration tests < 2 min. E2E tests < 5 min. Coverage Ã¢â€°Â¥ 80% for modified module. Zero flaky tests (pass rate 100% over 5 consecutive runs). |
...
| **Current Status** | Ã°Å¸â€Â´ Not testable Ã¢â‚¬â€ test frameworks configured (Jest, Vitest, Playwright) but zero tests written. CI Postgres service container ready. |

---

## Scenario 8: Reliability Ã¢â‚¬â€ API Service Restart
...
| **Artifact** | NestJS API Docker container Ã¢â€ â€™ Docker daemon Ã¢â€ â€™ PrismaService Ã¢â€ â€™ BullMQ queue |
...
| **Current Status** | Ã°Å¸Å¸Â¡ Partially testable Ã¢â‚¬â€ Docker restart policy configured in `infrastructure/docker/docker-compose.yml`. BullMQ job recovery depends on Redis lock mechanism. |

---

## Scenario 9: Security Ã¢â‚¬â€ SQL Injection Attempt
...
| **Artifact** | Vercel Edge Ã¢â€ â€™ NestJS API Ã¢â€ â€™ PrismaService Ã¢â€ â€™ Prisma ORM Ã¢â€ â€™ Supabase PostgreSQL |
| **Environment** | Production. Attacker is external, unauthenticated. |
| **Response** | Prisma ORM parameterizes all queries Ã¢â‚¬â€ the slug value is treated as a string literal, never interpolated into SQL. The query resolves to `SELECT * FROM projects WHERE slug = $1` with the malicious string as the parameter value. The query returns zero rows (no matching slug). The API returns a 200 with `{"data":[],"meta":{"total":0}}`. No SQL injection occurs. The logging interceptor records the request. |
...
| **Verification Method** | Integration test: `request(app.getHttpServer()).get('/api/portfolio/projects/1;DROP TABLE projects;--').expect(200)` Ã¢â‚¬â€ verification query counts rows before and after (should be identical). |
| **Current Status** | Ã°Å¸Å¸Â¢ Secured by design Ã¢â‚¬â€ Prisma ORM uses parameterized queries exclusively; raw SQL is never executed |

---

## Scenario 10: Performance Ã¢â‚¬â€ AI Response Streaming
...
| **Artifact** | FastAPI Ã¢â€ â€™ LangChain Ã¢â€ â€™ RAG service (pgvector) Ã¢â€ â€™ OpenAI GPT-4o Ã¢â€ â€™ SSE stream to browser |
...
| **Current Status** | Ã°Å¸â€Â´ Not testable Ã¢â‚¬â€ AI service not implemented |

---

## Scenario 11: Maintainability Ã¢â‚¬â€ Documentation Update
...
| **Current Status** | Ã°Å¸Å¸Â¢ Document structure is modular with clear TOC, consistent sectioning, and Mermaid diagrams |

---

## Scenario 12: Availability Ã¢â‚¬â€ CDN Cache Serving During API Outage
...
| **Artifact** | Vercel CDN Ã¢â€ â€™ Next.js ISR cache (stale) |
...
| **Response** | Vercel CDN serves the cached (stale) version of the homepage and projects page. The pages render fully with previously cached content. Background revalidation attempts fail silently Ã¢â‚¬â€ no error is shown to the visitor. Sentry captures the revalidation failure as a warning. The pages continue serving stale content until the API recovers and the next revalidation succeeds. |
...
| **Current Status** | Ã°Å¸Å¸Â¡ Designed (ISR with stale-while-revalidate) Ã¢â‚¬â€ not verified via chaos testing |
...
|------|---------|--------|---------|
| July 2026 | 1.0 | CTO | Initial scenarios Ã¢â‚¬â€ 12 scenarios covering 8 quality attributes |
