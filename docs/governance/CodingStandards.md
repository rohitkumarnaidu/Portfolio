...
2. **`src/portfolio/controllers/<entity>.controller.ts`** — Public, read-only endpoints (`/api/portfolio/...`). Unauthenticated, `@CacheTTL` applied, response-cached. Registered in `PortfolioModule`.
...
- **Generation:** Always run `npm run prisma:generate` after schema changes (custom output path: `apps/api/generated/prisma`).
- **Migrations:** `prisma migrate dev` for local, `prisma migrate deploy` for production.
- **Access:** Through `PrismaService` (`src/common/database/prisma.service.ts`), which wraps a `PrismaClient` built on the `pg` driver adapter.
...
- **Pre-commit:** Husky runs lint-staged (Prettier + ESLint --fix) on all staged files. Do not bypass with `--no-verify`.
...
- Dependencies array must be exhaustive (enforced by `react-hooks/exhaustive-deps` ESLint rule).
- `useCallback` and `useMemo` only when profiling shows a performance issue — not preemptively.
