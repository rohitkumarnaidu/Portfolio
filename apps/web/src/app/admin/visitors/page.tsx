...
function DailyViewsChart({
  ...
  const points = data.map((d, i) => {
    ...
    const yVisitors = padding.top + chartH - (d.visitors / maxVal) * chartH;
    const yViews = padding.top + chartH - (d.page_views / maxVal) * chartH;
  ...
  return (
    ...
    >
      ...
      <g transform={`translate(${width - padding.right - 10}, ${padding.top})`}>
        <circle cx="0" cy="0" r="3" fill="var(--accent-500)" />
        <text x="8" y="4" className="fill-text-tertiary text-[10px]" fontSize="10">
          Visitors
        </text>
        <line
          x1="0"
          ...
          strokeDasharray="4 3"
        />
        <text x="8" y="20" className="fill-text-tertiary text-[10px]" fontSize="10">
          Views
        </text>
      </g>
    </svg>
...
export default function VisitorsPage() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([getAnalyticsSummary(period), getAnalyticsSessions({ per_page: 200 })])
      .then(([s, sess]) => {
        setSummary(s);
        setSessions(sess);
      })
  ...
  sessions.forEach((s) => {
    if (s.referrer) {
      try {
        ...
        countryCount.set(host, (countryCount.get(host) || 0) + 1);
      } catch { /* empty */ }
    }
  });
  ...
  if (loading && !summary)
    return (
      <div>
        <h1 className="font-display text-h2 text-text-primary mb-8">Visitors</h1>
        {loadingEl}
      </div>
    );
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-display text-h2 text-text-primary">Visitors</h1>
          <p className="text-body-sm text-text-secondary mt-1">
            Real-time visitor intelligence and breakdowns
          </p>
        </div>
      </div>
      ...
      <div className="rounded-2xl bg-surface-secondary border border-border-primary p-5 mb-6">
        ...
        {summary?.visitors_over_time ? (
          <DailyViewsChart data={summary.visitors_over_time} />
        ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div className="rounded-2xl bg-surface-secondary border border-border-primary p-5">
          <h3 className="font-display text-h4 text-text-primary mb-4">Referral Sources</h3>
          <BarList items={referrers} />
        </div>
        <div className="rounded-2xl bg-surface-secondary border border-border-primary p-5">
          <h3 className="font-display text-h4 text-text-primary mb-4">Referrer Domains</h3>
