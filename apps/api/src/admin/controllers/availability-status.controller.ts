...
import { UpdateAvailabilityStatusDto } from '../../modules/availability-status/dto';
...
export class AdminAvailabilityStatusController {
  ...
  @Get() @Roles('admin', 'editor', 'viewer') @ApiOperation({ summary: 'Get availability status' })
  async getStatus() {
    return { data: await this.service.getStatus() };
  }

  @Patch() @Roles('admin', 'editor') @Audit({ action: 'update', resource: 'availability_status' }) @ApiOperation({ summary: 'Update availability status' })
