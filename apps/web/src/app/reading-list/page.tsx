      ...
      <div className="max-w-6xl mx-auto px-4 py-16 md:py-24">
        <div className="mb-12">
          ...
          <p className="text-body text-text-secondary max-w-2xl">
            Books, articles, and resources I&apos;m reading or recommend.
          </p>
        </div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className="rounded-2xl bg-surface-secondary border border-border-primary p-6 animate-pulse h-32"
              />
            ))}
          </div>
        ) : !items?.length ? (
            ...
            <div key={category} className="mb-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {categoryItems.map((item) => (
                  <div
                    key={item.id}
                    className="rounded-2xl bg-surface-secondary border border-border-primary p-5 hover:border-border-accent transition-colors"
                  >
