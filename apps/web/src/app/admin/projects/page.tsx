    ...
    try {
      await Promise.all(
        Array.from(selectedIds).map((id) =>
          updateProject.mutateAsync({ id, data: { is_featured: featured } }),
        ),
      );
      addToast({
        variant: 'success',
        title: `${selectedIds.size} projects ${featured ? 'featured' : 'unfeatured'}`,
      });
      setSelectedIds(new Set());
    } catch {
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          <p className="text-body text-text-secondary">No projects found. Create your first one!</p>
        </div>
      ) : (
        <div>
