...
interface AuditEvent {
  ...
  changes: {                     // What changed (for mutations)
    field: string;
    old_value: any;
    new_value: any;
  }[];
...
interface Notification {
  id: string;
  type: 'new_lead' | 'error_alert' | 'performance_alert' | 'weekly_digest' | 'daily_digest' | 'system_update' | 'permission_change' | 'uptime_alert' | 'deployment_status';
...
Some aggregated data displayed in admin dashboards depends on **background job completion**. The following background jobs (see `docs/api/47-BACKGROUND-JOBS.md`) affect data freshness:
...
The following compliance gates from `docs/security/16-COMPLIANCE.md §12` apply specifically to admin dashboard deployment:
...
All error states across the admin dashboard use the **standardized error codes** defined in `docs/api/ErrorHandling.md` and the **API response envelope** from `docs/api/44-API-STANDARDS.md`. Every API error response follows this structure:
...
|-----------|-------------|
| `docs/architecture/SystemArchitecture.md` (v5.0) | System architecture — admin service integration |
| `docs/database/DatabaseArchitecture.md` (v5.0) | Database schema — all admin-related tables (leads, sections, audit_logs, etc.) |
| `docs/api/12-API.md` (v5.0) | API documentation — admin endpoints, auth, rate limiting |
| `docs/security/SecurityArchitecture.md` (v5.0) | Security implementation — OWASP Top 10:2025 for admin |
| `docs/security/15-AUTHORIZATION.md` (v5.0) | Authorization — JWT, RBAC, permission model |
| `docs/security/16-COMPLIANCE.md` (v5.0) | Compliance — GDPR, CCPA, WCAG, SOC 2 for admin |
| `docs/operations/AnalyticsArchitecture.md` (v5.0) | Analytics — PostHog integration for admin dashboards |
| `docs/operations/21-MONITORING.md` (v5.0) | Monitoring — Sentry, uptime, alerting for admin |
| `docs/operations/22-OBSERVABILITY.md` (v5.0) | Observability — admin monitoring widget specs |
| `docs/design/DesignSystem.md` (v5.0) | Design system — admin component catalog (DataTable, StatsCard, Charts) |
| `docs/design/DesignSystem.md` (v5.0) | UI/UX architecture — admin-specific patterns (§15) |
| `docs/product/02-FEATURES.md` (v3.0) | Feature catalog — F-400 series (Admin Dashboard), F-600 series (CMS) |
| `docs/product/03-USER-STORIES.md` (v3.0) | User stories — E2 (Admin Content), E3 (Leads), E4 (Auth), E5 (Analytics) |
| `docs/product/37-IMPLEMENTATION_PLAN.md` (v5.0) | Implementation plan — Phase 9: Admin Dashboard (12 days) |
| `docs/security/43-DATA-GOVERNANCE.md` (v1.0) | Data governance — admin data retention, GDPR |
| `docs/api/44-API-STANDARDS.md` (v5.0) | API standards — REST conventions, error codes, pagination |
| `docs/api/ErrorHandling.md` (v1.0) | Error catalog — standardized error codes for admin API |
| `docs/api/46-EVENT-ARCHITECTURE.md` (v5.0) | Event architecture — admin audit events |
| `docs/operations/22-OBSERVABILITY.md` (v5.0) | Observability — admin dashboard metrics |
| `docs/operations/54-INFRASTRUCTURE.md` (v1.0) | Infrastructure — admin system deployment topology |
