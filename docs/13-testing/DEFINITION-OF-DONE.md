...
## 10. DoD Enforcement Automation

- **PR template:** Includes a DoD checklist that must be completed before merge.
- **CI gate:** Automated checks verify code formatting, linting, type checking, and test execution.
- **Coverage gate:** PRs that reduce overall test coverage by more than 2% are blocked.
- **Security gate:** PRs with new CodeQL findings (error or warning) are blocked.
- **Label enforcement:** PRs must have appropriate labels (feat, fix, chore) matching branch prefix.

## 11. DoD for AI-Generated Code

- [ ] AI-generated code is reviewed with extra scrutiny for hallucinations and security issues.
- [ ] All AI-generated test cases are verified to actually run and pass.
- [ ] AI-generated documentation is fact-checked against actual code behavior.
- [ ] AI-generated code follows the same coding standards as human-written code.

## 12. DoD Quick Reference Card

| Work Type      | Must-Have                                         | Nice-to-Have            |
| -------------- | ------------------------------------------------- | ----------------------- |
| Feature        | Code review, tests, docs, a11y, perf              | Feature flag, analytics |
| Bug Fix        | Root cause doc, regression test, staging verify   | Post-mortem             |
| Refactor       | Tests pass, no warnings, behavior preserved       | Migration guide         |
| Docs           | Accuracy check, cross-refs valid, examples tested | Screenshots             |
| Infrastructure | IaC, rollback plan, monitoring, alerting          | Load test               |
