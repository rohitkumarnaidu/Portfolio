...
export function DataTable<T>({
  data, columns, keyExtractor, isLoading = false,
  pageSize = 10, searchable = false, searchPlaceholder = 'Search...',
  emptyState, onEdit, onDelete, bulkActions = [],
}: DataTableProps<T>) {
  ...
  const [bulkConfirm, setBulkConfirm] = useState<{ action: () => Promise<void> | void; title: string; message: string } | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    if (search && searchable) {
      ...
      result = data.filter(item => {
        const str = columns.map(c => String(c.render(item, 0) ?? '')).join(' ').toLowerCase();
  ...
  const allSelected = paged.length > 0 && paged.every(item => selectedIds.has(keyExtractor(item)));
  ...
  const handleBulkAction = (ba: typeof bulkActions[0]) => {
    ...
    const message = ba.confirmMessage ? ba.confirmMessage(ids.length) : `Are you sure you want to ${ba.label.toLowerCase()} ${ids.length} items?`;
  ...
  return (
    <div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {paged.map((item, idx) => {
              ...
              return (
                <div key={id} className="flex items-center gap-4 p-4 rounded-xl bg-surface-secondary border border-border-primary hover:border-border-accent transition-colors">
                  {(onEdit || onDelete || bulkActions.length > 0) && (
                    <label className="flex items-center justify-center w-5 h-5 shrink-0">
                      <input type="checkbox" checked={selectedIds.has(id)} onChange={() => toggleSelect(id)} className="rounded border-border-primary" />
                    </label>
                  )}
                  {columns.map(col => (
                    <div
                  ...
                  {(onEdit || onDelete) && (
                    <div className="flex items-center gap-1 shrink-0">
                      {onEdit && (
                        <button onClick={() => onEdit(item)} className="w-8 h-8 rounded-lg flex items-center justify-center text-text-tertiary hover:text-text-primary hover:bg-surface-elevated transition-colors" aria-label="Edit">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                        </button>
                      )}
                      {onDelete && (
                        <button onClick={() => setDeleteTarget(item)} className="w-8 h-8 rounded-lg flex items-center justify-center text-text-tertiary hover:text-semantic-error hover:bg-semantic-error-bg transition-colors" aria-label="Delete">
      ...
      <Modal isOpen={!!bulkConfirm} onClose={() => setBulkConfirm(null)} title={bulkConfirm?.title || ''} size="sm">
