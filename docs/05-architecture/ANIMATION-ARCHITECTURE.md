# Animation Architecture — FAANG Enterprise Motion System
...
This document defines the comprehensive enterprise FAANG-level motion design system — including zero-jank animation principles, dynamic easing curves, precise transition durations, hardware-accelerated parallax effects, and hyper-optimized scroll-triggered animations. Strict performance budgets guarantee smooth UI motion at 60fps on mid-range devices while maintaining 100% reduced-motion compliance.
...
**Motion Philosophy:** _"Motion should communicate, not decorate."_
...
:root {
  ...
  --ease-in-out: cubic-bezier(0.4, 0, 0.2, 1); /* Transition both */
  --ease-linear: cubic-bezier(0, 0, 1, 1); /* Continuous */
...
export const useScrollParallax = (ref: React.RefObject<HTMLDivElement>, speed: number = 0.3) => {
  useEffect(() => {
